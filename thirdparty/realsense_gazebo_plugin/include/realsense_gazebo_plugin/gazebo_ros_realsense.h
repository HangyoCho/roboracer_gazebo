...
protected:
  ...
  /// \brief Pre-computed tan lookup tables for point cloud (avoid per-pixel atan2/tan)
  std::vector<float> tanX_, tanY_;
  bool pcLutReady_ = false;
  void InitPointCloudLUT(uint32_t rows, uint32_t cols, double hfov);
};
}
