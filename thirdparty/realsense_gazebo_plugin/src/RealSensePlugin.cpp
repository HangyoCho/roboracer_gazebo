...
void RealSensePlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  do {
    ...
    if (name == "depthUpdateRate")
      ...
      cameraParamsMap_[IRED1_CAMERA_NAME].optical_frame =
          _sdf->GetValue()->GetAsString();
    else if (name == "depthNoiseBase")
      _sdf->GetValue()->Get(depthNoiseBase_);
    else if (name == "depthNoiseSlope")
      _sdf->GetValue()->Get(depthNoiseSlope_);
    else
      throw std::runtime_error("Invalid parameter for RealSensePlugin: " + name);

    _sdf = _sdf->GetNextElement();
  } while (_sdf);

  // Enable distance-dependent noise if parameters are set
  if (depthNoiseBase_ > 0.0f || depthNoiseSlope_ > 0.0f) {
    useDistanceNoise_ = true;
    std::cout << "RealSensePlugin: Distance-dependent depth noise enabled: "
              << "stddev(d) = " << depthNoiseBase_ << " + " << depthNoiseSlope_
              << " * d" << std::endl;
    noiseGen_.seed(std::random_device{}());
  }
  ...
  // ired1 and ired2 are optional (L515 can run without IR sensors)
  ...
  if (!this->ired1Cam) {
    std::cout << "RealSensePlugin: InfraRed Camera 1 not found (IR disabled)"
              << std::endl;
  }
  if (!this->ired2Cam) {
    std::cout << "RealSensePlugin: InfraRed Camera 2 not found (single IR mode, e.g. L515)"
              << std::endl;
  }
  if (!this->colorCam) {
  ...
  try {
    unsigned int imgSize = this->depthCam->ImageWidth() *
                           this->depthCam->ImageHeight();
    this->depthMap.resize(imgSize);
    this->noisyDepthData.resize(imgSize);
...
void RealSensePlugin::OnNewDepthFrame() {
  ...
  for (unsigned int i = 0; i < imageSize; ++i) {
    float depth = depthDataFloat[i];

    // Apply distance-dependent noise: stddev(d) = base + slope * d
    if (useDistanceNoise_ && depth > 0 && depth >= rangeMinDepth_ &&
        depth <= rangeMaxDepth_) {
      float stddev = depthNoiseBase_ + depthNoiseSlope_ * depth;
      depth += stdNormal_(noiseGen_) * stddev;
    }

    // Store noisy float depth for point cloud generation
    this->noisyDepthData[i] = depth;

    // Check clipping and overflow
    if (depth < rangeMinDepth_ ||
        depth > rangeMaxDepth_ ||
        depth > DEPTH_SCALE_M * UINT16_MAX ||
        depth < 0) {
      this->depthMap[i] = 0;
    } else {
      this->depthMap[i] = (uint16_t)(depth / DEPTH_SCALE_M);
    }
  }
