...
// Flag to indicate whether ired2 camera is present (D435 has two IR, L515 has one)
#define HAS_IRED2(ptr) ((ptr) != nullptr)
...
protected:
  ...
  /// \brief Store noisy float depth data for point cloud generation.
  std::vector<float> noisyDepthData;
  ...
  /// \brief Distance-dependent depth noise: stddev(d) = base + slope * d
  bool useDistanceNoise_ = false;
  float depthNoiseBase_ = 0.0f;
  float depthNoiseSlope_ = 0.0f;
  std::default_random_engine noiseGen_;
  std::normal_distribution<float> stdNormal_{0.0f, 1.0f};
};
}
#endif
